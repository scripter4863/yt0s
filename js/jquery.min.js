...
function renderDetail(vData) {
    ...
    // }
    if(k_page!='home'){
        detail_html += '<iframe scrolling="no" src="https://yt-mp3s.me/button/'+k_page+'/'+vData.vid+'&amp;bg=E91E63" style="width:100%;height:60px;border:0px;"></iframe> &nbsp &nbsp' +
        '<a id="cnext" class="form-control mesg-convert " href="'+k_url_next+'">Convert next</a></div><br/><br/>';
    }else{
        detail_html+='<span style="display:block"><iframe scrolling="no" src="https://yt-mp3s.me/button/mp3/'+vData.vid+'&amp;bg=E91E63" style="width:100%;height:60px;border:0px;"></span></iframe><iframe scrolling="no" src="https://yt-mp3s.me/mp4/'+vData.vid+'" style="width:100%;height:60px;border:0px;"></iframe><a id="cnext" class="form-control mesg-convert " href="'+k_url_next+'">Convert next</a><br/><br/>';
    }
   
        detail_html+='<div class="addthis_inline_share_toolbox"></div>'+
        ...
        if(links[ltype] !== undefined){
            ...
            $.each(qs, function (stt, fquality) {
                $.each(links[ltype], function (itag, finfo) {
                    if(fquality == finfo.q){
                        e_html += '<option value="'+finfo.k+'">' + ((typeof finfo.q_text !='undefined' && finfo.q_text)?finfo.q_text:finfo.q) + (finfo.size?' (' + finfo.size +')':'') +' </option>';
                    }
                });
            });
            e_html += '</optgroup>';
                ...
                if(sdata.status == 'ok'){
                    if(sdata.p === 'search'){
                        ...
                        if(typeof sdata.q!='undefined' && sdata.q!=''){
                            var h_url = k_url_next+"?q="+encodeURIComponent(sdata.q);
